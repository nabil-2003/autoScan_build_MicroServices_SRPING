...
class ProjectScanner:
            ...
            if 'spring' in data:
                ...
                if 'datasource' in spring:
                    ...
                    if 'url' in ds:
                        ...
                        if 'postgresql' in db_url:
                            ...
                            db_name_match = re.search(r'postgresql://[^:/]+(?::(\d+))?/([^?]+)', db_url) or \
                                          re.search(r'jdbc:postgresql://[^:/]+(?::(\d+))?/([^?]+)', db_url)
                            if db_name_match:
                                # Extract port if present (group 1), database name (group 2)
                                if db_name_match.group(1):  # Port found
                                    info['db_port'] = int(db_name_match.group(1))
                                info['db_name'] = db_name_match.group(2)
                        elif 'mysql' in db_url:
                            info.setdefault('dependencies', set()).add('mysql')
                            # Extract database name and optional port from URL
                            db_name_match = re.search(r'mysql://[^:/]+(?::(\d+))?/([^?]+)', db_url) or \
                                          re.search(r'jdbc:mysql://[^:/]+(?::(\d+))?/([^?]+)', db_url)
                            if db_name_match:
                                # Extract port if present (group 1), database name (group 2)
                                if db_name_match.group(1):  # Port found
                                    info['db_port'] = int(db_name_match.group(1))
                ...
                if 'kafka' in spring:
                    ...
                    if 'bootstrap-servers' in kafka:
                        ...
                        info['kafka_bootstrap_servers'] = bootstrap
                        # Extract port from bootstrap servers (e.g., "localhost:9999" or "kafka:9092")
                        port_match = re.search(r':(\d+)', bootstrap)
                        if port_match:
                            info['kafka_port'] = int(port_match.group(1))
                ...
                if 'data' in spring and 'mongodb' in spring['data']:
                    ...
                    if 'uri' in mongo:
                        uri = mongo['uri']
                        info['mongodb_uri'] = uri
                        # Extract port from URI (e.g., "mongodb://user:pass@localhost:27888/db")
                        port_match = re.search(r'@[^:]+:(\d+)', uri) or re.search(r'mongodb://[^:]+:(\d+)', uri)
                        if port_match:
                            info['mongodb_port'] = int(port_match.group(1))
    ...
    def parse_properties_config(self, props_path: Path) -> Dict:
        """Parse properties configuration file and extract ALL relevant configurations (ignoring comments)"""
        info = {}
        
        try:
            content = props_path.read_text(encoding='utf-8')
            
            # Extract server port (ignore commented lines)
            port_match = re.search(r'^(?!\s*[#!])\s*server\.port\s*=\s*(\d+)', content, re.MULTILINE)
            if port_match:
                info['port'] = int(port_match.group(1))
            
            # Extract application name (ignore commented lines)
            name_match = re.search(r'^(?!\s*[#!])\s*spring\.application\.name\s*=\s*(.+)$', content, re.MULTILINE)
            if name_match:
                info['app_name'] = name_match.group(1).strip()
            
            # Extract datasource configuration (ignore commented lines)
            url_match = re.search(r'^(?!\s*[#!])\s*spring\.datasource\.url\s*=\s*(.+)$', content, re.MULTILINE)
            if url_match:
                ...
                if 'postgresql' in db_url:
                    info.setdefault('dependencies', set()).add('postgres')
                    # Extract database name and optional port from URL
                    db_name_match = re.search(r'postgresql://[^:/]+(?::(\d+))?/([^?\s]+)', db_url) or \
                                  re.search(r'jdbc:postgresql://[^:/]+(?::(\d+))?/([^?\s]+)', db_url)
                    if db_name_match:
                        # Extract port if present (group 1), database name (group 2)
                        if db_name_match.group(1):  # Port found
                            info['db_port'] = int(db_name_match.group(1))
                    ...
                    info.setdefault('dependencies', set()).add('mysql')
                    # Extract database name and optional port from URL
                    db_name_match = re.search(r'mysql://[^:/]+(?::(\d+))?/([^?\s]+)', db_url) or \
                                  re.search(r'jdbc:mysql://[^:/]+(?::(\d+))?/([^?\s]+)', db_url)
                    if db_name_match:
                        # Extract port if present (group 1), database name (group 2)
                        if db_name_match.group(1):  # Port found
                            info['db_port'] = int(db_name_match.group(1))
            ...
            # Extract database username (ignore commented lines)
            user_match = re.search(r'^(?!\s*[#!])\s*spring\.datasource\.username\s*=\s*(.+)$', content, re.MULTILINE)
            if user_match:
                info['db_user'] = user_match.group(1).strip()
            
            # Extract database password (ignore commented lines)
            pass_match = re.search(r'^(?!\s*[#!])\s*spring\.datasource\.password\s*=\s*(.+)$', content, re.MULTILINE)
            if pass_match:
                info['db_password'] = pass_match.group(1).strip()
            
            # Extract Kafka configuration (ignore commented lines)
            kafka_bootstrap = re.search(r'^(?!\s*[#!])\s*spring\.kafka\.bootstrap-servers\s*=\s*(.+)$', content, re.MULTILINE)
            if kafka_bootstrap:
                bootstrap = kafka_bootstrap.group(1).strip()
                info.setdefault('dependencies', set()).add('kafka')  # Mark Kafka as dependency
                info['kafka_bootstrap_servers'] = bootstrap
                # Extract port from bootstrap servers (e.g., "localhost:9999" or "kafka:9092")
                port_match = re.search(r':(\d+)', bootstrap)
            ...
            kafka_group = re.search(r'^(?!\s*[#!])\s*spring\.kafka\.consumer\.group-id\s*=\s*(.+)$', content, re.MULTILINE)
            if kafka_group:
                info['kafka_consumer_group'] = kafka_group.group(1).strip()
            
            # Extract Redis configuration (ignore commented lines)
            redis_host = re.search(r'^(?!\s*[#!])\s*spring\.redis\.host\s*=\s*(.+)$', content, re.MULTILINE)
            if redis_host:
                info.setdefault('dependencies', set()).add('redis')  # Mark Redis as dependency
                info['redis_host'] = redis_host.group(1).strip()
            
            redis_port = re.search(r'^(?!\s*[#!])\s*spring\.redis\.port\s*=\s*(\d+)', content, re.MULTILINE)
            if redis_port:
                info['redis_port'] = int(redis_port.group(1))
            
            redis_pass = re.search(r'^(?!\s*[#!])\s*spring\.redis\.password\s*=\s*(.+)$', content, re.MULTILINE)
            if redis_pass:
                info['redis_password'] = redis_pass.group(1).strip()
            
            # Extract MongoDB configuration (ignore commented lines)
            mongo_uri = re.search(r'^(?!\s*[#!])\s*spring\.data\.mongodb\.uri\s*=\s*(.+)$', content, re.MULTILINE)
            if mongo_uri:
                uri = mongo_uri.group(1).strip()
                info.setdefault('dependencies', set()).add('mongodb')  # Mark MongoDB as dependency
                info['mongodb_uri'] = uri
                # Extract port from URI (e.g., "mongodb://user:pass@localhost:27888/db")
                port_match = re.search(r'@[^:]+:(\d+)', uri) or re.search(r'mongodb://[^:]+:(\d+)', uri)
            ...
            mongo_db = re.search(r'^(?!\s*[#!])\s*spring\.data\.mongodb\.database\s*=\s*(.+)$', content, re.MULTILINE)
            if mongo_db:
                info['mongodb_database'] = mongo_db.group(1).strip()
            
            # Extract RabbitMQ configuration (ignore commented lines)
            rabbit_host = re.search(r'^(?!\s*[#!])\s*spring\.rabbitmq\.host\s*=\s*(.+)$', content, re.MULTILINE)
            if rabbit_host:
                info.setdefault('dependencies', set()).add('rabbitmq')  # Mark RabbitMQ as dependency
                info['rabbitmq_host'] = rabbit_host.group(1).strip()
            
            rabbit_port = re.search(r'^(?!\s*[#!])\s*spring\.rabbitmq\.port\s*=\s*(\d+)', content, re.MULTILINE)
            if rabbit_port:
                info['rabbitmq_port'] = int(rabbit_port.group(1))
            
            rabbit_user = re.search(r'^(?!\s*[#!])\s*spring\.rabbitmq\.username\s*=\s*(.+)$', content, re.MULTILINE)
            if rabbit_user:
                info['rabbitmq_user'] = rabbit_user.group(1).strip()
            
            rabbit_pass = re.search(r'^(?!\s*[#!])\s*spring\.rabbitmq\.password\s*=\s*(.+)$', content, re.MULTILINE)
            if rabbit_pass:
                info['rabbitmq_password'] = rabbit_pass.group(1).strip()
            
            # Extract Eureka configuration (ignore commented lines)
            eureka_url = re.search(r'^(?!\s*[#!])\s*eureka\.client\.service-url\.defaultZone\s*=\s*(.+)$', content, re.MULTILINE)
            if eureka_url:
                info['eureka_url'] = eureka_url.group(1).strip()
            
            eureka_hostname = re.search(r'^(?!\s*[#!])\s*eureka\.instance\.hostname\s*=\s*(.+)$', content, re.MULTILINE)
            if eureka_hostname:
                info['eureka_instance_hostname'] = eureka_hostname.group(1).strip()
            
            # Extract Config Server (ignore commented lines)
            config_uri = re.search(r'^(?!\s*[#!])\s*spring\.cloud\.config\.uri\s*=\s*(.+)$', content, re.MULTILINE)
            if config_uri:
...
"""
    ...
    def collect_db_credentials(self) -> Dict[str, Dict[str, str]]:
        """Collect database credentials and ports from all services"""
        credentials = {
            'mysql': {},
            'postgres': {}
        }
        ...
        for service_info in self.scanner.services.values():
            # Collect MySQL credentials and port
            if 'mysql' in service_info['dependencies']:
                ...
                if 'db_port' in service_info:
                    credentials['mysql']['port'] = str(service_info['db_port'])
            
            # Collect PostgreSQL credentials and port
            if 'postgres' in service_info['dependencies']:
                ...
                if 'db_port' in service_info:
                    credentials['postgres']['port'] = str(service_info['db_port'])
    ...
    def generate_postgres_service(self) -> str:
        ...
        # Use actual credentials and port from config files, or defaults if not found
        ...
        db_port = postgres_creds.get('port', '5432')
...
"""
    ...
    def generate_mysql_service(self) -> str:
        ...
        db_port = mysql_creds.get('port', '3306')
...
"""
    ...
    def generate_mongodb_service(self) -> str:
        ...
        # Collect actual MongoDB credentials and port from services
        mongo_user = 'admin'
        mongo_pass = 'admin'
        mongo_db = 'testdb'
        mongo_port = '27017'
        ...
        for service_info in self.scanner.services.values():
            if 'mongodb' in service_info['dependencies']:
                ...
                if 'mongodb_port' in service_info:
                    mongo_port = str(service_info['mongodb_port'])
...
"""
    ...
    def generate_redis_service(self) -> str:
        ...
        # Collect actual Redis password and port from services
        redis_pass = None
        redis_port = '6379'
        ...
        for service_info in self.scanner.services.values():
            if 'redis' in service_info['dependencies']:
                ...
                if 'redis_port' in service_info:
                    redis_port = str(service_info['redis_port'])
                break
        
        self.env_vars.append(f"REDIS_PORT={redis_port}")
        if redis_pass:
...
"""
    ...
    def generate_kafka_service(self) -> str:
        ...
        # Collect actual Kafka port from services
        kafka_port = '9092'
        
        for service_info in self.scanner.services.values():
            if 'kafka' in service_info['dependencies']:
                if 'kafka_port' in service_info:
                    kafka_port = str(service_info['kafka_port'])
                break
...
"""
    ...
    def generate_rabbitmq_service(self) -> str:
        ...
        # Collect actual RabbitMQ credentials and port from services
        rabbit_user = 'guest'
        rabbit_pass = 'guest'
        rabbit_port = '5672'
        ...
        for service_info in self.scanner.services.values():
            if 'rabbitmq' in service_info['dependencies']:
                ...
                if 'rabbitmq_port' in service_info:
                    rabbit_port = str(service_info['rabbitmq_port'])
                break
        
        self.env_vars.extend([
            f"RABBITMQ_USER={rabbit_user}",
            f"RABBITMQ_PASSWORD={rabbit_pass}",
            f"RABBITMQ_PORT={rabbit_port}",
...
"""
    ...
    def generate_service_block(self, service_name: str, service_info: Dict) -> str:
        ...
        if 'mysql' in dependencies or 'mysql' in self.scanner.dependencies:
            ...
            db_port = service_info.get('db_port', 3306)
            
            block += f"      SPRING_DATASOURCE_URL: jdbc:mysql://mysql:{db_port}/${{MYSQL_DATABASE:-{db_name}}}\n"
            ...
            db_port = service_info.get('db_port', 5432)
            
            block += f"      SPRING_DATASOURCE_URL: jdbc:postgresql://postgres:{db_port}/${{DB_NAME:-{db_name}}}\n"
  ...
  python auto-docker-generator.py C:\\Projects\\my-microservices
    ...
    if not args.skip_version_check and scanner.services:
        ...
        if is_consistent:
            ...
            print(f"\n💡 Suggested fix — align all pom.xml files to these versions:")
            if 'spring_boot' in target_versions:
                print(f"   Spring Boot  →  {target_versions['spring_boot']}")
                print(f"      <parent>")
                print(f"          <artifactId>spring-boot-starter-parent</artifactId>")
                print(f"          <version>{target_versions['spring_boot']}</version>")
                print(f"      </parent>")
            if 'spring_cloud' in target_versions:
                print(f"   Spring Cloud →  {target_versions['spring_cloud']}")
                print(f"      <spring-cloud.version>{target_versions['spring_cloud']}</spring-cloud.version>")
            if 'java' in target_versions:
                print(f"   Java         →  {target_versions['java']}")
                print(f"      <java.version>{target_versions['java']}</java.version>")

            print(f"\n⚠️  Continuing with original versions — no files were modified.")
